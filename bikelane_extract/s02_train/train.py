"""train — OpenSatMap tile pairs → U-Net (ResNet34) lane-marking segmentation.

Ported from 04_train_segmentation. Classes: 0 bg, 1 lane line, 2 curb,
3 virtual line; 255 = ignore. Ground truth is 1 px wide, so masks are
dilated (train.dilate_px) and the loss is CE (bg weight 0.1) + Dice.

Data layout (produced by `bikelane prepare`):
...
Checkpoints: <ckpt_dir>/last.pt, best.pt ({'model','epoch','miou'}), best by
mIoU over classes 1–3. Copy best.pt to weights/seg_unet_r34.pt for `predict`.
...
from .common import IGNORE, IMAGENET_MEAN, IMAGENET_STD, make_model
...
def run(cfg: Config, argv=None, check: bool = False):
    import argparse
    ap = argparse.ArgumentParser(prog="bikelane train")
    ...
    print(f"  done. best mIoU {best:.3f} → copy {ckpt_dir/'best.pt'} to {cfg.weights('seg')}")
