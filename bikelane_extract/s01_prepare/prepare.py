"""prepare — OpenSatMap level-20 download → baked masks → 1024 px image/GT tile pairs.

Ported from 01_prepare_data and 03_bake_masks. This stage drives the
OpenSatMap `tools-release` scripts (external repo), so it needs:
...
def run(cfg: Config, argv=None, check: bool = False):
    import argparse
    ap = argparse.ArgumentParser(prog="bikelane prepare")
